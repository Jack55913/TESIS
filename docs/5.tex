...
\begin{figure}[h!]
\centering
  \includegraphics[width=0.5\textwidth]{publicidad/4.jpg}
  \caption{Escala de graduación para precipitación. Fuente: Fotografía propia (2022)}
  % Nótese que en la parte superior se ve una línea gruesa, esta es por el aceite; además que el agua ya superó la escala de graduación, en estos casos el equipo capacitado hace el vaciado y contempla ese volumen extra, además que el aceite es rescatado en un recipiente para evitar contaminación.}
		\label{publicidad4}
\end{figure}
\begin{figure}[h!]
\centering
...
\begin{figure}[ht]
\centering
  \includegraphics[width=0.6\textwidth]{m12.jpg}
  \caption{Medición de espacio entre arboles. Fuente: Fotografía propia (2022)}
  \label{m12}
\end{figure}
\begin{figure}[ht]
\centering
  \includegraphics[width=0.6\textwidth]{m13.jpg}
  \caption{Medición de altura de árboles con un altímetro. Fuente: Fotografía propia (2022)}
  \label{m13}
\end{figure}
...
\textit{\begin{tabular}[c]{@{}c@{}}Largo\\ del   \\ paraje (m)\end{tabular}} &
  ...
  134 &
  191 \\
\textit{Condición de barrera de árboles} &
  \begin{tabular}[c]{@{}c@{}}Parcial,\\ con claros\\ de viento\end{tabular} &
  \begin{tabular}[c]{@{}c@{}}Bosque\\ denso,\\ poco\\ expuesto\end{tabular} &
  \begin{tabular}[c]{@{}c@{}}Bosque\\ abierto,\\ disperso\end{tabular} &
  \begin{tabular}[c]{@{}c@{}}Barrera\\ completa\\ de aprox\\ 20 m\end{tabular} &
  \begin{tabular}[c]{@{}c@{}}Abierto,\\ con\\ exposición\\ al viento\end{tabular} &
  \begin{tabular}[c]{@{}c@{}}75\%\\ rodeado,\\ viento\\ presente\end{tabular} &
  \begin{tabular}[c]{@{}c@{}}Homogéneo,\\ cima de\\ lomerío\end{tabular} &
  \begin{tabular}[c]{@{}c@{}}Masa forestal\\ cerrada\\ y laderas\\ envolventes\end{tabular} \\
\textit{Accesibilidad} &
  Acceso fácil &
  Acceso fácil &
  Camino bueno &
  Acceso vehicular &
  \begin{tabular}[c]{@{}c@{}}Acceso\\ moderado\\ en seco\end{tabular} &
  \begin{tabular}[c]{@{}c@{}}Camioneta\\ en seco,\\ difícil en lluvia\end{tabular} &
  \begin{tabular}[c]{@{}c@{}}A pie de\\ camino,\\ muy accesible\end{tabular} &
  \begin{tabular}[c]{@{}c@{}}Subida difícil,\\ acceso caminando\\ 500 m\end{tabular} \\
\textit{Tipo de monitoreo} &
  ...
  Ejidal &
  Mixto \\
\multicolumn{9}{c}{Pendientes Menores al 19°} \\
\bottomrule
\end{tabular}%
}
\caption{Resultados de la selección de los sitios de monitoreo.  Fuente: Elaboración propia (2022)}
\label{tabrsm1}
\end{table}
\end{landscape}
...
\begin{figure}[h!]
\centering
  \includegraphics[width=0.8\textwidth]{m14.jpg}
...
\begin{figure}[h!]
\centering
  \includegraphics[width=0.8\textwidth]{m11.jpg}
...
% Se muestra la versión final de uno de los carteles utilizados (Figura \ref{fig:cartel1}). Este incluye información básica sobre la aplicación, la invitación a participar en el monitoreo ciudadano y un código QR para descargar la app desde la tienda de aplicaciones. Estos carteles fueron colocados en sitios estratégicos dentro del monte.

% \begin{figure}[h!]
% \centering
%   \includegraphics[width=0.6\textwidth]{publicidad/1.jpg}
...
% Se muestra una de las lonas utilizadas en las reuniones comunitarias (Figura \ref{fig:lona}). El diseño mantiene la línea gráfica del proyecto, pero con un mayor nivel de detalle y adaptaciones para la visibilidad a distancia.
...
% Se visualiza uno de los diseños impresos del tríptico (Figura \ref{fig:triptico}). Se empleó un lenguaje accesible, diagramas explicativos y un diseño adaptable a impresión económica a color o blanco y negro.
...
% Se enseña el cartel utilizado para anunciar la entrega de obsequios, acompañado de una pequeña ceremonia comunitaria (Figura \ref{fig:obsequios}).
...
Posteriormente el sitio se podrá encontrar accesible desde un dominio personalizado con HTTPS habilitado automáticamente por Firebase Hosting, pudiéndose administrar sus versiones en el panel (Figura \ref{m8}).
\begin{figure}[ht]
\centering
...
Cuando la aplicación es iniciada por primera vez, la función main %(Anexo \ref{anexo:alg3}) 
el cual llama al StatelessWidget \texttt{MyApp} 
%(Anexo \ref{anexo:alg4})
, que a su vez invoca el StatelessWidget \texttt{ConditionalOnboardingPage} %(Anexo \ref{anexo:alg5})
. Este ultimo widget es un condicional que determina si la pantalla inicial es el Onboarding %(Anexo \ref{anexo:alg6}) 
o el HomePage. %(Anexo \ref{anexo:alg7}).

El Onboarding (Figura \ref{pantallas1}) es importante porque da la bienvenida al usuario y es la carátula del proyecto. Una vez completada esta sección, se navega a la pantalla de iniciar sesión con cuenta Google, la cual se le llamó \texttt{SignUpWidget} %(Anexo \ref{anexo:alg8})
...
El diseño de la pantalla de inicio de sesión, %anexo \ref{anexo:alg8}, 
...
\newpage
\subsubsection*{Login}
El modelo de la pantalla (Figura \ref{pantallas2})%, código adjunto al anexo \ref{anexo:alg9}
...
En la página CommonSelectPage() %(Anexo \ref{anexo:alg10})
...
La clase \texttt{AddScreen} %(anexo \ref{anexo:alg13}) 
...
La clase \texttt{DataScreen} %(anexo \ref{anexo:alg14}) 
...
\draw [arrow] (saveAsReal.east) -- ++(1.5,0) -- ++(0,-9) -- (end.east);
