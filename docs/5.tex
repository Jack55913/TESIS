...
\subsection{Estructura documental del proyecto en Flutter}
...
% El sistema implementa un esquema de cuatro capas de seguridad. La primera capa establece un permiso genérico que restringe todas las operaciones a usuarios autenticados con al menos un rol asignado (request.auth.token.roles.size() > 0), cumpliendo así con el requisito básico de autenticación.

% La segunda capa, demarcada como "Rowy rules", introduce funciones reutilizables (colRule, hasAnyRole) que validan accesos a colecciones específicas mediante verificaciones de roles. Por ejemplo, la regla colRule(["roles"], ["ADMIN","EDITOR","VIEWER","OWNER"]) permite operaciones CRUD solo si el usuario posee alguno de los roles especificados y la colección objetivo está en la lista blanca definida. Este enfoque sigue el patrón de listas de control de acceso (ACLs) adaptativo descrito por Ferraiolo y Kuhn (2001).

% En la tercera capa, se implementan reglas personalizadas para el dominio de mediciones pluviométricas. Estas incorporan condiciones híbridas que combinan roles privilegiados (ADMIN/OWNER) con propiedad del dato (request.auth.uid == resource.data.uploader_id). Para las colecciones measurements y real_measurements, se permite lectura pública autenticada, mientras que la modificación requiere ser propietario del registro o tener privilegios administrativos. Esta estrategia refleja el modelo RBAC-A (Role-Based Access Control with Attributes) propuesto por Jin et al. (2012).

% La cuarta capa gestiona configuraciones del sistema mediante reglas especializadas. El segmento _rowy_/schema utiliza funciones dinámicas (canModify) que consultan metadatos almacenados en Firestore para determinar permisos, demostrando un enfoque de seguridad basado en políticas (Policy-Based Access Control). Las reglas en _rowy_/publicSettings, por contraste, ilustran una excepción deliberada al permitir lectura anónima (allow get: if true), necesaria para funcionalidades públicas.

% Funciones de Utilidad
% Tres funciones centralizan la lógica de autorización:

% hasAnyRole(roles) verifica membresía en conjuntos de roles, siguiendo el estándar NIST RBAC (Sandhu et al., 2000).

% isDocOwner(docId) implementa el patrón de ownership mediante comparación de UIDs, crítico para el principio de autogestión de datos.

% canModify(tableId, tableType) demuestra autorización contextual al consultar configuraciones almacenadas, técnica recomendada por Google Cloud para sistemas multiinquilino.

% De este modo, se garantiza la privacidad de los datos y se previene la modificación no autorizada.
...
\subsubsection{1. Instalación de Firebase CLI}
...
\subsubsection{4. Compilación de la app Flutter para la web}
...
\begin{figure}[h!]
\centering
  \includegraphics[width=0.6\textwidth]{assets/pantallas/1.pdf}
...
\begin{figure}[h!]
\centering
  \includegraphics[width=0.6\textwidth]{assets/pantallas/2.pdf}
...
\subsubsection*{Lógica del Código QR:}
...
\begin{figure}[h!]
\centering
  \includegraphics[width=0.6\textwidth]{assets/pantallas/3.pdf}
...
\begin{figure}[h!]
\centering
  \includegraphics[width=0.6\textwidth]{assets/pantallas/5.pdf}
...
\begin{figure}[h!]
\centering
  \includegraphics[width=0.6\textwidth]{assets/pantallas/7.pdf}
...
\begin{figure}[h!]
\centering
  \includegraphics[width=0.6\textwidth]{assets/pantallas/8.pdf}
...
\begin{figure}[h!]
\centering
  \includegraphics[width=0.6\textwidth]{assets/pantallas/10.pdf}
...
\begin{figure}[h!]
\centering
  \includegraphics[width=0.6\textwidth]{assets/pantallas/12.pdf}
...
\begin{figure}[h!]
\centering
  \includegraphics[width=0.6\textwidth]{assets/pantallas/13.pdf}
...
\begin{figure}[h!]
\centering
  \includegraphics[width=0.6\textwidth]{assets/pantallas/14.pdf}
  \caption{Pantalla de estadísticas: Gráfica de volúmenes reales }
  \label{pantallas14}
\end{figure}
...
% \subsubsection{Diseño visual y coherencia (Material Design)}
% Describe cómo los componentes visuales siguen las guías de Material Design para ofrecer una interfaz intuitiva, predecible y estética.

% \subsubsection{Accesibilidad}
% Explica si se consideró el contraste de colores, uso de íconos comprensibles, textos legibles y compatibilidad con lectores de pantalla.

% \subsubsection{Flujo de navegación}
% Detalla cómo fue diseñado el flujo lógico entre pantallas: qué tan fácil es registrar una medición desde que el usuario abre la app, cómo vuelve al menú, etc.

% \subsubsection{Retroalimentación del sistema}
% Indica si la app ofrece mensajes claros cuando el usuario guarda una medición, comete un error o no hay conexión.

% \subsubsection{Distribución y acceso (Tienda de aplicaciones)}
% Describe cómo se facilita la instalación de la app desde Play Store o web, incluyendo aspectos como actualizaciones automáticas y peso ligero del archivo.

% \subsubsection{Motivación y compromiso del usuario}
% Incluye si hay elementos que motivan el uso continuo (por ejemplo, contador de mediciones, mensajes de agradecimiento, inclusión de grupos de monitoreo, etc.)
